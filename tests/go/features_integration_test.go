...
func TestIntegrationFeatureSuitePreviewsWithGopls(t *testing.T) {
	...
	assertWorkspaceSymbol(t, symbols, "message", mainURI)

	formatEdit := previewFormatting(t, mgr, mainFile, text)
	assertTextEditForURI(t, "formatting", formatEdit, mainURI)
	...
	edits := assertTextEditForURI(t, "rename", renameEdit, mainURI)
...
func previewFormatting(t *testing.T, mgr *lsp.Manager, absPath, text string) lsp.WorkspaceEdit {
	...
	for range goFeatureLookup.Attempts {
		...
		if lastErr == nil && workspaceEditHasTextEdits(edit) {
			return edit
		}
		waitForFeature(t)
	}
...
func assertRangeFormattingPreviewOrUnsupported(t *testing.T, mgr *lsp.Manager, absPath, text, wantURI string, rng protocol.Range) {
	...
	assertTextEditForURI(t, "range formatting", edit, wantURI)
}
...
func assertWorkspaceSymbol(t *testing.T, symbols []lsp.WorkspaceSymbol, wantName, wantURI string) {
	t.Helper()
...
func assertTextEditForURI(t *testing.T, label string, edit lsp.WorkspaceEdit, wantURI string) []lsp.WorkspaceTextEdit {
	t.Helper()
