...
// language server. It builds the command, materializes a fixture module from a
// txtar archive, and connects as a real MCP client over a subprocess transport,
// so it exercises the same path an agent uses rather than the in-memory fakes
// the mcp package unit tests rely on.
...
// command against a real gopls over a subprocess MCP transport, so it is opt-in
// and excluded from a plain `go test ./...`.
...
// TestIntegrationGopls drives the mcp-lsp -serve command as a real MCP client,
// over a subprocess stdio transport, against a live gopls. It proves the full
// path an agent uses: the MCP handshake, tools/list, and a tools/call that
// reaches gopls through the LSP client and returns structured diagnostics.
//
// The test is opt-in: it builds the command, writes a fixture module to disk,
// and spawns gopls, so it is skipped unless MCP_LSP_INTEGRATION is set and gopls
// is on PATH. It is also skipped under -short.
func TestIntegrationGopls(t *testing.T) {
	if testing.Short() {
		t.Skip("skipping integration test under -short")
	}
	if os.Getenv(integrationEnv) == "" {
		t.Skipf("set %s=1 to run the live gopls integration test", integrationEnv)
	}
	...
	cmd := exec.Command(bin, "-serve", "--", "gopls", "serve")
	...
	// broken.go references an undefined identifier, so gopls type-checking must
	// report exactly one error-severity diagnostic at its position. clean.go
	// compiles, so both models must report none. The position is one-based to
	// match how the tool flattens LSP ranges.
...
// writeFixtureModule materializes the embedded txtar fixture into a temporary
// directory and returns its path. gopls only type-checks inside a real module
// on disk, so the archive's files are written out rather than served from an
// in-memory fs.FS.
